...
export const Mutaties: _i_signatures.Mutaties = ($, $p) => ({
    'Bankrekening Mutatie Verwerkingen': _ea.cc($.Bankrekeningen, ($) => sh_g.wrap_dictionary($.map(($) => sh_g.wrap_dictionary($.Mutaties.filter(($) => _ea.cc($.Status, ($): _et.Optional_Value<_i_out.Mutaties.Bankrekening_Mutatie_Verwerkingen.D.D<_ed.Source_Location>> => {
        switch ($[0]) {
